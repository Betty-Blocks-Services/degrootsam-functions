(() => ({
  ...
  jsx: (() => {
    ...
    let localStorageYear = localStorage.getItem('currentSelectedYear');

    useEffect(() => {
      if (useCurrentYearValue) {
        if (localStorageYear === null) {
          const currentYear = new Date().getUTCFullYear();
          localStorage.setItem('currentSelectedYear', currentYear);
          localStorageYear = currentYear;
        }
      }
    }, []);
    ...
    const getDefaultValue = () =>
      useCurrentYearValue ? localStorageYear : resolvedCurrentValue;

    const [currentValue, setCurrentValue] = usePageState(getDefaultValue());

    useEffect(() => {
      B.triggerEvent('onRender', getDefaultValue());
    }, []);
    ...
    useEffect(() => {
      B.defineFunction('Reset', () => setCurrentValue(getDefaultValue()));
    }, []);

    useEffect(() => {
      if (mounted.current) {
        localStorage.setItem('currentSelectedYear', currentValue);
    ...
    const idOrPathValue =
      typeof valueProperty.id !== 'undefined'
        ? valueProperty.id
        : valueProperty;

    const getValue = (row) => {
      if (valueProperty) {
        const prop = getProperty(idOrPathValue[0]);
    ...
    useEffect(() => {
      if (!isDev && !loading && !currentValue && useDefaultValue) {
        if (valueType === 'manual') {
          const array = manualValue.split('\n').map((v) => v.trim());
          setCurrentValue(array[0]);
        } else if (data) {
          const rows = (data && data.results) || data || [];
          setCurrentValue(
            getValue(
              useCurrentYearValue
                ? rows.find((e) => e.year === localStorageYear)
                : rows[0],
            ),
          );
        }
    ...
    if (!isListProperty && !isDev && valueType !== 'manual') {
      if (!modelId) {
    ...
    const renderOptions = () => {
      ...
      if (!loading && !isDev) {
        ...
        return rows.map((row) => (
          <MenuItem key={row.id} value={getValue(row)}>
            {renderLabel(row)}
          </MenuItem>
        ));
      }
    ...
    const SelectCmp = (
      <FormControl fullWidth={fullWidth}>
        ...
        >
          ...
          {placeholderLabelText && !getDefaultValue() && (
